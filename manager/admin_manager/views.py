...
def admin_dashboard(request):
    ...
    if request.method == 'GET':
        ...
        # select box
        context['admin_lv'] = '<td><table width="100%"><tr>'
        context['admin_lv'] += '<td><select id="manage_lv">' \
        ...
        for page in page_list:
            ...
            context['page_auth'] += '<td>' \
                                    '<p>{}</p>' \
                                    '<p><input type="checkbox" name="{}" value="r" />{}</p>' \
                                    '<p><input type="checkbox" name="{}" value="u" />{}</p>' \
                                    '<p><input type="checkbox" name="{}" value="d" />{}</p></td>' \
